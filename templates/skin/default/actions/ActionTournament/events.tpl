...
{if $oStreamEvent->getEventType()=="match_played"}
	...
	{assign var=oBlogs value=$oTarget->getBlog()}
			<span class="date" title="{date_format date=$oStreamEvent->getDateAdd()}">{date_format date=$oStreamEvent->getDateAdd() hours_back="12" minutes_back="60" now="60" day="day H:i" format="j F Y, H:i"} ({$aLang.plugin.vs.match} №{$oTarget->getNumber()}) {if $oBlogs}<a href="{$oTournament->getUrlFull()}match_comment/{$oTarget->getMatchId()}" target="_blank" >{$aLang.plugin.vs.game_report} ({$oTarget->getCountComment()})</a> {/if}{if $oTarget->getWithVideo() ==1}<i class="icon-facetime-video"></i>{/if}</span> 

			<table id="myTable" width="100%">
<tbody>
<tr class="odd" height="10">
<td class="mid" width="40" align="left">{if $oAwayTeam}<img width="32" src="{cfg name='path.root.web'}/images/teams/small/{$oAwayTeam->getLogo()}" alt="" align="absmiddle">{/if}{if !$oAwayTeam && $oAwayUser}<img width="32" src="{$oAwayUser->getProfileAvatarPath(48)}" alt="avatar" class="avatar" />{/if}</td> 
  ...
  <td class="mid" align="center" colspan="2" width="60"><b><font color="#184e8d" size="4">{$oTarget->getLeftGoals()}:{$oTarget->getRightGoals()} {$oTarget->getAdditionalResult()}</font> <font color="#184e8d"></font></b></td>
  ...
  <td class="mid" width="40" align="right">{if $oHomeTeam}<img width="32" src="{cfg name='path.root.web'}/images/teams/small/{$oHomeTeam->getLogo()}" alt="" align="absmiddle">{/if}{if !$oHomeTeam && $oHomeUser}<img width="32" src="{$oHomeUser->getProfileAvatarPath(48)}" alt="avatar" class="avatar" />{/if}</td> 
</tr>
<tr>

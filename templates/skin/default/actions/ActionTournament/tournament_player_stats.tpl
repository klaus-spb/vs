...
<div class="block-content" id="div_updates">
{*
<ul class="nav nav-pills">						
	<li {if $who=='all'}class="active"{/if}><a href="{$oTournament->getUrlFull()}player_stats/">{$aLang.plugin.vs.all}</a></li>
	<li {if $who=='attack'}class="active"{/if}><a href="{$oTournament->getUrlFull()}player_stats/attack/">{$aLang.plugin.vs.forwards}</a></li>
	<li {if $who=='defence'}class="active"{/if}><a href="{$oTournament->getUrlFull()}player_stats/defence/">{$aLang.plugin.vs.dmen}</a></li>
	<li {if $who=='goalkeeper'}class="active"{/if}><a href="{$oTournament->getUrlFull()}player_stats/goalkeeper/">{$aLang.plugin.vs.goalies}</a></li>
	<li {if $who=='situation'}class="active"{/if}><a href="{$oTournament->getUrlFull()}player_stats/situation/">{$aLang.plugin.vs.goalsit}</a></li>
</ul>
*}
<small>
...
<tr {if $oUserCurrent && $oUserCurrent->getUserId()==$oStat.user_id}class="my"{/if}> 
	<td width="20" align="center">{$num}</td>
	<td width="250" align="left"><span class="goalauthor"><a href="{router page='profile'}{$oStat.user_login}/" class="stream-author">{$oStat.user_login}</a> <span class="fio">{$oStat.fio}</span></td> 
	<td width="37">{if $oStat.team_id>0}<a href="{$oTeam->getUrlFull()}"> <img style="height:20px;" src="{cfg name='path.root.web'}/images/teams/{if $oStat.user_id==0}small{else}teamplay{/if}/{$oStat.team_logo}"/></a>{/if}</td>
	<td align="center">{if $oStat.team_id>0}<a href="{$oTeam->getUrlFull()}" class="teamrasp">{$oStat.team_brief}</a>{/if}</td>
...
<ul> 
	...
	<li>L1 — Lead by 1 / при преимуществе в 1 шайбу</li>
	<li>L2 — Lead by 2 / при преимуществе в 2 шайбы</li>
	<li>L3+ — Lead by 3+ / при преимуществе в 3 шайбы и больше</li>

...
{*<div class="turnirmenu">
	<ul>
		<li class="active"><a href="{$link}"><span class="bgl"></span><span class="bgi">{$aLang.plugin.in_blogs}</span><span class="bgr"></span></a></li>
		<li><a href="{$link_blogiturnirov}"><span class="bgl"></span><span class="bgi">{$aLang.plugin.events}</span><span class="bgr"></span></a></li>
...
{include file="$sTemplatePathPlugin/actions/ActionVs/gametype_menu.tpl"  whats="index"}
...
<div class="blog">
	...
	<ul class="actions">  
		...
		{if $oUserCurrent->getRating()>=$oBlog->getLimitRatingTopic() and ($oUserCurrent->getId()==$oBlog->getOwnerId() or $oBlog->getUserIsJoin() or $oUserCurrent->isAdministrator() or $oBlog->getUserIsAdministrator())}  
				<li><a href="{router page='topic'}add/{$oBlog->getId()}" title="{$aLang.plugin.vs.create_topic}" id="blog_delete_show" class="edit">{$aLang.plugin.vs.create_topic}</a></li>
		{/if}
		{/if}
	</ul>

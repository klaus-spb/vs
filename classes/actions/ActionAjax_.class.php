...
class PluginVs_ActionAjax extends PluginVs_Inherit_ActionAjax {	
	...
	protected function EventGetTalk(){
		...
		*/
		...
		$sTextResult=$oViewer->Fetch(Plugin::GetTemplatePath(__CLASS__)."blocks/block.talk.tpl");
	...
	protected function EvenAddMatchVideo(){ 
		...
		if (func_check(getRequest('video_url',null,'post'),'text',2,300))$video_url=getRequest('video_url',null,'post');
		
		$title = 'Видео матча №'.$oMatch->getNumber().' между '.$oMatch->getAwayteam()->getName().' и '.$oMatch->getHometeam()->getName();
		$text = 'Видео к матчу турнира '.$oMatch->getTournament()->getName().' матча №'.$oMatch->getNumber().' между '.$oMatch->getAwayteam()->getName().' и '.$oMatch->getHometeam()->getName();
		$oTopic=Engine::GetEntity('Topic');
		$oTopic->_setValidateScenario('video');
		$oTopic->setBlogId($oMatch->getBlogId());
		$oTopic->setTitle(strip_tags($title));
		$oTopic->setTextSource($text);
		$oTopic->setTags("видео к матчу");
		...
		$this->Hook_Run('topic_add_after', array('oTopic'=>$oTopic));
	...
	protected function EventGoFrom(){ 
		...
		if($who=='player' && $oPlayertournament->getTeamId()!=0 && ($oPlayer->getTeamId() == $oPlayertournament->getTeamId() || $this->oUserCurrent->getIsAdministrator()) ){
			...
			if($oPlayertournament = $this->PluginVs_Stat_GetPlayertournamentItemsByFilter(array(
				...
				{	
					...
					$oTalk = $this->Talk_SendTalk('Игрок покинул клуб', '', $this->oUserCurrent, $aUsers);
				}
		...
		if($who=='team' && $oPlayertournament->getTeamId()!=0 ){
			...
			$oTalk=$this->Talk_SendTalk('Вы исключены из клуба', '', $this->oUserCurrent,$aUsers);
				...
				)))
			{
				if($oPlayer->getTeamId()==$oPlayerTournament->getTeamId() || $this->oUserCurrent->getIsAdministrator()){
					if($id!=2){
						...
						$oPlayerTournament->Save();
						$this->Message_AddNoticeSingle(" ","Сохранено");
					}
					...
					$this->Message_AddErrorSingle('Игрок не в вашей команде',$this->Lang_Get('error'));
					return;				
				}
			
			}else{
				$this->Message_AddErrorSingle('Нет такого игрока',$this->Lang_Get('error'));
				return;				
			}
		}else{
			$this->Message_AddErrorSingle('У вас нет таких прав',$this->Lang_Get('error'));
			return;			
		}
	}
	protected function EventAnswerInvite(){
		...
		if($oInvite->getTournamentId()==0){
			if($who=='player' && $oInvite->getPlayerSubmit()==0){
				if($id==1){				
					...
					$oPlayerTournament->setCap(0);
					$oPlayerTournament->Save();	
					$answer='Игрок '.$oPlayercard->getFullFio().' вступил в клуб';
					$why='';
				}
				if($id==-1){				
					$oInvite->setPlayerSubmit(-1);
					...
					$answer='Игрок '.$oPlayercard->getFullFio().' отверг приглашение клуб';
					if($why!='')$why='Причина отказа "'.$why.'"';
				}
					$aUsers=array();
			...
			if($who=='team' && $oInvite->setTeamSubmit()==0){
				if($id==1){	
					...
					$oPlayerTournament->setCap(0);
					$oPlayerTournament->Save();	
					$answer=''.$oPlayercard->getFullFio().' принят в клуб';
					$why='';
				}
				if($id==-1){				
					$oInvite->setTeamSubmit(-1);
					...
					$answer='Клуб отказал в приеме '.$oPlayercard->getFullFio().'';
					if($why!='')$why='Причина отказа "'.$why.'"';
				}
					$aUsers=array();
		...
		$this->Message_AddNoticeSingle(" ","Сделано");		
	}
    ...
    protected function EventIntoTournament() {
		...
		if(!$oPlayercard = $this->PluginVs_Stat_GetPlayercardByFilter(array(
			...
			$this->Message_AddErrorSingle('Сначала создайте карточку игрока',$this->Lang_Get('error'));
            return;			
		}
	...
	protected function EventMyBlogs() {
		...
		$sTextResult=$oViewer->Fetch(Plugin::GetTemplatePath(__CLASS__)."blocks/block.my_blogs.tpl"); 
		$this->Viewer_AssignAjax('sText',$sTextResult);
	}
	...
	protected function EventTeamsTeamplay() {
		...
		$sTextResult=$oViewer->Fetch(Plugin::GetTemplatePath(__CLASS__)."blocks/block.teams_teamplay.tpl"); 
		$this->Viewer_AssignAjax('sText',$sTextResult);
...
and pt.tournament_id=t.tournament_id
 ";
		...
		$sTextResult=$oViewer->Fetch(Plugin::GetTemplatePath(__CLASS__)."blocks/block.my_matches.tpl"); 
		$this->Viewer_AssignAjax('sText',$sTextResult);
	}
	protected function EventMyTournaments() {
		...
		$sTextResult=$oViewer->Fetch(Plugin::GetTemplatePath(__CLASS__)."blocks/block.my_turnirs.tpl"); 
		$this->Viewer_AssignAjax('sText',$sTextResult);
	...
	protected function EventStreamMatches() {
		...
		if ($aComments=$this->Comment_GetCommentsOnline('match',$stream_count)) {
			$oViewer=$this->Viewer_GetLocalViewer();
			$oViewer->Assign('aComments',$aComments);
			$sTextResult=$oViewer->Fetch(Plugin::GetTemplatePath(__CLASS__)."blocks/block.stream_matches.tpl");
			$this->Viewer_AssignAjax('sText',$sTextResult);
		} else {
			$this->Message_AddErrorSingle($this->Lang_Get('block_stream_comments_no'),$this->Lang_Get('attention'));
			return;
...
protected function EventResultGonkaSet () {
	if (func_check(getRequest('kval_id',null,'post'),'id',1,11))$kval_id=getRequest('kval_id',null,'post');
		...
		$this->DeletePlayerStatGonka($kval_id);	
		$oZaezd=$this->PluginVs_Stat_GetZaezdByZaezdId($kval_id) ;
		$oZaezd->setBestLap($best_lap);
		$oZaezd->setTimes($times);
		$oZaezd->setPoints($points);
		$oZaezd->Save();
		$this->CalcPlayerStatGonka($kval_id);
		$this->CalcPositionGonka($kval_id);
}
...
if($oTournament){		
		...
		$oViewer->Assign('link_match_insert',$oBlog->getUrlFull()."turnir/".$oTournament->getUrl()."/_match_insert/");	
		$oViewer->Assign('link_turnir', $oBlog->getUrlFull()."turnir/".$oTournament->getUrl());
	...
	protected function EventDayInfo() {
		...
		if(isset($team[0])){
			...
			$oViewer->Assign('link_match', DIR_WEB_ROOT."/turnir/".$oTournament->getUrl()."/_match/");
		
			$sTextResult=$oViewer->Fetch("day_info.tpl");
			$this->Viewer_AssignAjax('sText',$sTextResult);
		}
	...
	protected function EventTeamInfo() {
		...
		if($oTournament->getGametypeId()!=3){	
			...
			if($game_id!=0 && $gametype_id!=0 && $tournament_id==0){
						...
						$oViewer->Assign('oUser',$oUser);
						$sTextResult=$oViewer->Fetch("team_info.tpl");
						$this->Viewer_AssignAjax('sText',$sTextResult);
			}
	...
	protected function EventRaspisanieMonth() {
	  ...
	  $sTextResult=$oViewer->Fetch("blocks/block.turnir_teamrasp.tpl");
	  $this->Viewer_AssignAjax('sText',$sTextResult);
	...
	protected function EventAutocompleterPlayercard() {
		...
		if( $aPlayers = $this->PluginVs_Stat_GetPlayercardItemsByFilter(array(
				'#where' => array("(concat(family,' ', name) like ? )" => array("%".$sValue."%"))			
				))
			)
		{
			foreach ($aPlayers as $oPlayer) {
				$aItems[]=$oPlayer->getFamily().' '.$oPlayer->getName();
			}
			$this->Viewer_AssignAjax('aItems',$aItems);
		}
	}
	...
	protected function CalcPlayerStat($match_id) {
		$oMatch = $this->PluginVs_Stat_GetMatchesByMatchId($match_id);
	...
	protected function DeletePlayerStat($match_id) {
		$oMatch = $this->PluginVs_Stat_GetMatchesByMatchId($match_id);
	...
	protected function CalcStat($match_id) {
		$oMatch = $this->PluginVs_Stat_GetMatchesByMatchId($match_id);
		if($oMatch->getTournamentId()!=0){
	...
	protected function CalcPosition($match_id) {
	...
	protected function CalcNewRating($match_id) {
		$oMatch = $this->PluginVs_Stat_GetMatchesByMatchId($match_id);
	...
	protected function CalcRating($match_id) {
		$oMatch = $this->PluginVs_Stat_GetMatchesByMatchId($match_id);
...
*/	
	...
	protected function EventRaspisanieMonthes() 	{
		...
		if (func_check(getRequest('tournament',null,'post'),'id',1,3))$tournament_id=getRequest('tournament',null,'post');
		$month=-1;
		if (func_check(getRequest('monthes',null,'post'),'id',1,2))$month=getRequest('monthes',null,'post');
...
$oViewer->Assign('oGame',$oGame);			
	...
	protected function EventAuRaspisanie() {
		...
		$sTextResult=$oViewer->Fetch("au_raspisanie.tpl");
		$this->Viewer_AssignAjax('sText',$sTextResult);
	...
	protected function EventAuTournament() {
			...
			$sTextResult=$oViewer->Fetch("au_tournament.tpl");
			$this->Viewer_AssignAjax('sText',$sTextResult);
	...
	protected function EventAuAddTeam() {
		...
		if (func_check(getRequest('team',null,'post'),'text',1,40))$team=getRequest('team',null,'post');
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			$oTournament=$this->PluginVs_Stat_GetTournamentByTournamentId($tournament_id);
			$data = explode(":", $team);
	...
	protected function EventAuChange() {
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			$oViewer->Assign('oUser',$oUser);
			$sTextResult=$oViewer->Fetch("au_changeteam.tpl");
			$this->Viewer_AssignAjax('sText',$sTextResult);
		}else {
			$this->Message_AddErrorSingle('вы не админ',$this->Lang_Get('attention'));
			return;
	...
	protected function EventAuZayavki() {
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			$oViewer->Assign('oUser',$oUser);
			$sTextResult=$oViewer->Fetch("au_zayavki.tpl");
			$this->Viewer_AssignAjax('sText',$sTextResult);
		}else {
			$this->Message_AddErrorSingle('вы не админ',$this->Lang_Get('attention'));
			return;
	...
	protected function EventCreateRaspisanie() {
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			$sql="SELECT distinct 1	FROM `tis_stat_matches` WHERE tournament_id = ".$tournament_id." and round_id=1 and round_po='".$rounds[$round]."' ";
	...
	protected function EventAuGroup() {
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			$sTextResult=$oViewer->Fetch("au_group.tpl");
			$this->Viewer_AssignAjax('sText',$sTextResult);
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			if( $aRoundPosition = $this->PluginVs_Stat_GetAll($sql)
					)
				{
					...
					$sql="SELECT distinct 1	FROM `tis_stat_matches` WHERE tournament_id = ".$tournament_id." and round_id=1 and round_po='1' ";
			...
			if( $aRoundPosition = $this->PluginVs_Stat_GetAll($sql)
					)
				{
					...
					$sql="SELECT distinct 1	FROM `tis_stat_matches` WHERE tournament_id = ".$tournament_id." and round_id=1 and round_po='1/2' ";
			...
			if( $aRoundPosition = $this->PluginVs_Stat_GetAll($sql)
					)
				{
					...
					$sql="SELECT distinct 1	FROM `tis_stat_matches` WHERE tournament_id = ".$tournament_id." and round_id=1 and round_po='1/4' ";
			...
			if( $aRoundPosition = $this->PluginVs_Stat_GetAll($sql)
					)
				{
					...
					$sql="SELECT distinct 1	FROM `tis_stat_matches` WHERE tournament_id = ".$tournament_id." and round_id=1 and round_po='1/8' ";
					if($this->PluginVs_Stat_GetAll($sql) )$oViewer->Assign('Matches8',1);
				}

			$sTextResult=$oViewer->Fetch("au_playoff.tpl");
			$this->Viewer_AssignAjax('sText',$sTextResult);
		} else {
			$this->Message_AddErrorSingle('вы не админ',$this->Lang_Get('attention'));
			return;
	...
	protected function EventAuTeams() {
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {		
			...
			$sTextResult=$oViewer->Fetch("au_teams.tpl");
			$this->Viewer_AssignAjax('sText',$sTextResult);
		} else {
			$this->Message_AddErrorSingle('вы не админ',$this->Lang_Get('attention'));
			return;
	...
	protected function EventAuPlayers() {
		...
		$aTeams = $this->PluginVs_Stat_GetTeamsintournamentItemsByFilter(array(
				...
				$sTextResult=$oViewer->Fetch("au_players.tpl");
				$this->Viewer_AssignAjax('sText',$sTextResult);
			} else {
				$this->Message_AddErrorSingle('вы не админ',$this->Lang_Get('attention'));
				return;
	...
	echo 'here6';
		...
		$this->Message_AddNoticeSingle(" ","Сохранено2");
	}
	...
	protected function EventSettingMatchTimeOtvet() { 
		...
		if($oMatchTime = $this->PluginVs_Stat_GetMatchtimeById($id) ){
			...
			if($whats=='accept'){
				...
				$title="Предложенное время '".$oMatchTime->getTimes()."' принято";
			...
			if($whats=='deny'){
				...
				$title="Предложенное время '".$oMatchTime->getTimes()."' отвергнуто";
			}
			...
			$oTalk=$this->Talk_SendTalk($title,'Для перехода на страницу согласования времени матча пройдите по <a href="'.$link.'">ссылке</a>',$this->oUserCurrent,$aUsers);
...
protected function EventCreateShedule() {
	...
	{		
		...
		foreach ($matches as $match ) {
			...
			if($match['team1']!=$secret_team && $match['team2']!=$secret_team){
				$oMatch_add =  Engine::GetEntity('PluginVs_Stat_Matches');			
				$oMatch_add->setTournamentId($tournament_id);
				...
				$oMatch_add->setHome($match['team1']);
				$oMatch_add->setAway($match['team2']);
				$oMatch_add->setHomeComment('');
				$oMatch_add->setAwayComment('');
				$oMatch_add->setNumber($num);
				$oMatch_add->setBlogId($oTournament->getBlogId());
				...
				if($sparenno==1){
					...
					$oMatch_add->setGametypeId($oTournament->getGametypeId());
					$oMatch_add->setDates(date("Y-m-d", $match['date']));
					$oMatch_add->setHome($match['team2']);
					$oMatch_add->setAway($match['team1']);
					$oMatch_add->setHomeComment('');
					$oMatch_add->setAwayComment('');
					$oMatch_add->setNumber($num);
					$oMatch_add->setBlogId($oTournament->getBlogId());
...
protected function EventCreateStatTable() {
	...
	protected function EventCreatePlayerStatTableByMatchId($match_id) {
		$oMatch = $this->PluginVs_Stat_GetMatchesByMatchId($match_id);
	...
	*/
	...
	protected function EventUploadLogo() {
		...
		if($tournament_id<>0){
			...
			if (isset($_FILES['tournament_'.$logo_type]) and is_uploaded_file($_FILES['tournament_'.$logo_type]['tmp_name'])) {
				...
				if ($sPath=$this->UploadLogo($_FILES['tournament_'.$logo_type], $oTournament->getUrl(),$logo_type, 'tournament')) {
		...
		if($blog_id<>0){
			...
			if (isset($_FILES['blog_'.$logo_type]) and is_uploaded_file($_FILES['blog_'.$logo_type]['tmp_name'])) {
				...
				if ($sPath=$this->UploadLogo($_FILES['blog_'.$logo_type], $oBlog->getUrl(),$logo_type, 'blog')) {
					if($logo_type=='small_logo'){
	...
	public function UploadLogo($aFile, $dir_name,$logo_type, $type)
    {
	...
	protected function EventUpdateTournamentRating() {
		...
		{		
			...
			foreach($aMatches as $oMatch)
			{
				//$this->CalcNewRating($oMatch->getMatchId()); 
				/*
				$this->CalcStat($oMatch->getMatchId());	
				$this->CalcPlayerStat($oMatch->getMatchId());	
...
//fwrite($fh, $oMatch->getMatchId().'-'); 
			...
			if($match_id!=0)$this->CalcPosition($match_id);
		}
	}

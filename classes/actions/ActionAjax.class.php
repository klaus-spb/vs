...
class PluginVs_ActionAjax extends PluginVs_Inherit_ActionAjax {	
	...
	protected function RegisterEvent() {	
		...
		$this->AddEventPreg('/^au$/i','/^penalty$/','EventAuPenalty'); //klaus
		
		$this->AddEventPreg('/^au$/i','/^addplayerpenalty$/','EventAuAddPenalty'); //klaus
		$this->AddEventPreg('/^au$/i','/^saveplayerpenalty$/','EventAuSavePenalty'); //klaus
		$this->AddEventPreg('/^au$/i','/^deleteplayerpenalty$/','EventAudeletePenalty'); //klaus
		
		$this->AddEventPreg('/^au$/i','/^award$/','EventAuAward'); //klaus
		
		$this->AddEventPreg('/^au$/i','/^addaward$/','EventAuAddAward'); //klaus
		$this->AddEventPreg('/^au$/i','/^saveaward$/','EventAuSaveAward'); //klaus
		$this->AddEventPreg('/^au$/i','/^deleteaward$/','EventAudeleteAward'); //klaus
		...
		$this->AddEventPreg('/^au$/i','/^event$/','EventAuEvent'); //klaus 
		$this->AddEventPreg('/^au$/i','/^addevent$/','EventAuAddEvent'); //klaus		
		$this->AddEventPreg('/^au$/i','/^editevent$/','EventAuEditEvent'); //klaus
		$this->AddEventPreg('/^au$/i','/^deleteevent$/','EventAuDeleteEvent'); //klaus
		$this->AddEventPreg('/^au$/i','/^calcevent$/','EventAuCalcEvent'); //klaus
		$this->AddEventPreg('/^au$/i','/^editeventpair$/','EventAuEditEventPair'); //klaus
		
		
		$this->AddEventPreg('/^au$/i','/^eventday$/','EventAuEventDay'); //klaus
		...
		$this->AddEventPreg('/^au$/i','/^deletenotplayed$/','EventDeleteNotPlayed'); //klaus
		...
		$this->AddEventPreg('/^au$/i','/^update_ratings$/','EventUpdateRatings'); //klaus
		...
		$this->AddEventPreg('/^raspisanie$/i','/^schedule_filter/','EventScheduleFilter'); //klaus
		$this->AddEventPreg('/^raspisanie$/i','/^schedule_my_filter/','EventScheduleMyFilter'); //klaus
		 
		$this->AddEvent('player_stat_filter','AjaxPlayerStatFilter');
		...
		$this->AddEventPreg('/^main$/i','/^streams$/','EventMainStreams');
		...
		$this->AddEventPreg('/^mainpage$/i','/^forum$/','EventMainpageForum');
		
	}
	
    protected function EventMainStreams() {
		
		if (false === ($aOnlines = $this->Cache_Get("main_streams"))) {
			
			$aOnlines = array();
			
			$sql = "SELECT u.user_login, fv.value

					FROM tis_user_field_value fv
					inner join tis_user u 
						on u.user_id = fv.user_id
						
					WHERE fv.field_id=20
					order by u.user_donator desc, u.user_rating desc";
			
			$aUsers=$this->PluginVs_Stat_GetAll($sql);
			if($aUsers){
				$aUserTwitch = array();
				$aUserToTwitch = array();
				foreach($aUsers as $aUser){
					$aUserTwitch[$aUser['value']] = $aUser['user_login'];
					$aUserToTwitch[] = $aUser['value'];
				}
				$limit = 100;
				$result = file_get_contents('https://api.twitch.tv/kraken/streams?limit='.$limit.'&channel='.implode(',',$aUserToTwitch));
				...
				foreach($result['streams'] as $stream) {
					...
					if (in_array($name,$aUserToTwitch)) {
						$index = array_search($name,$aUserToTwitch);
						unset($aUserToTwitch[$index]);
						$stream['channel']['user_login']=$aUserTwitch[$name];
						$aOnlines[] = $stream['channel'];
					}
				}
			}
			$this->Cache_Set($aOnlines, "main_streams", array(), 60);
		}
		
		$oViewer = $this->Viewer_GetLocalViewer();
		$oViewer->Assign('aOnlines', $aOnlines);
		$sTextResult = $oViewer->Fetch(Plugin::GetTemplatePath('vs').'widgets/widget.mainstreams_list.tpl');
		$this->Viewer_AssignAjax('sText', $sTextResult);
			
			
    }
	protected function EventMainpageForum(){
		
		if (func_check(getRequest('blog_id',null,'post'),'id',1,11))$blog_id=getRequest('blog_id',0,'post');
		 
		 
		$oForum = $this->Blog_GetBlogById($blog_id);
		...
		$sTextResult = $oViewer->Fetch(Plugin::GetTemplatePath('mainpage').'widgets/widget.mainpage_tab.tpl');
		$this->Viewer_AssignAjax('sText', $sTextResult);
        
	}
	
	protected function EventRating(){
		
		if (func_check(getRequest('game_id',null,'post'),'id',1,11))$game_id=getRequest('game_id',0,'post');
		if (func_check(getRequest('gametype_id',null,'post'),'id',1,11))$gametype_id=getRequest('gametype_id',0,'post');
		...
		if( $aRatings = $this->PluginVs_Stat_GetRatingItemsByFilter(array(
            ...
            // * ‘ормируем результат в виде шаблона и возвращаем
	...
	protected function AjaxPlayerStatFilter(){
		$dates=null;
		$who='all';
		$round = -1;
		if (func_check(getRequest('dates',null,'post'),'text',0,23))$dates=getRequest('dates',null,'post');
		if (func_check(getRequest('who',null,'post'),'text',0,23))$who=getRequest('who',null,'post');
		$teams=getRequest('teams',null,'post');
		if (func_check(getRequest('round',null,'post'),'id',0,4))$round=getRequest('round',null,'post');		
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',0,'post');
		
		$oTournament=$this->PluginVs_Stat_GetTournamentByTournamentId($tournament_id);
		
		if( $teams && is_array($teams)){ 
			foreach ($teams as $selectedTeam){
		...
		if( $dates ){
			...
			if( strlen($dates)==23 ){
				$aFilter['date_start'] = substr(strip_tags($dates), 0,10);
				$aFilter['date_end'] = substr(strip_tags($dates), 13,10);
				
				$aFilter['dates'] = strip_tags($dates);
			}
		}
		
		if($round != -1)$aFilter['round_id'] = $round;
		$aFilter['who'] = $who;
		...
		$oViewer->Assign('aStats',$aStats);
		
		foreach($aStats as $oStat){
			$ateams[] = $oStat['team_id'];
		}
		...
		$oViewer->Assign('who',$aFilter['who']);
		...
		$sTextResult=$oViewer->Fetch(Plugin::GetTemplatePath(__CLASS__)."actions/ActionTournament/player_stats.tpl");
		$this->Viewer_AssignAjax('sText',$sTextResult);
			
	}
	
	protected function EventScheduleMyFilter(){
		
		$dates=null;

		if (func_check(getRequest('dates',null,'post'),'text',0,23))$dates=getRequest('dates',null,'post');
		$players=getRequest('players',null,'post');
		$tournaments=getRequest('tournaments',null,'post');
		
		if( isset($tournaments) &&  $tournaments && is_array($tournaments)){ 
			foreach ($tournaments as $selectedTournament){
				$aFilter['tournaments'][] =  intval($selectedTournament);
			}
		}
		
		if( isset($players) &&  $players && is_array($players)){ 
			foreach ($players as $selectedPlayer){
		...
		$aFilter['month'] =0;
			...
			if(isset($aFilter['month']))$oViewer->Assign('month',$aFilter['month']);
			// $oViewer->Assign('tournament_id',$aFilter['tournament_id']);
			if(isset($aFilter['my_team_id']))$oViewer->Assign('myteam',$aFilter['my_team_id']);
	...
	protected function EventScheduleFilter(){
		...
		$teams=getRequest('teams',null,'post');
		$players=getRequest('players',null,'post');
		if (func_check(getRequest('not_played',null,'post'),'id',0,1))$not_played=getRequest('not_played',null,'post');		
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',0,'post');
		if (func_check(getRequest('round',null,'post'),'id',0,4))$round=getRequest('round',null,'post');
		if (func_check(getRequest('round_po',null,'post'),'text',0,6))$round_po=getRequest('round_po',null,'post');
		if (func_check(getRequest('event',null,'post'),'id',0,4))$event=getRequest('event',null,'post');
		
		$oTournament=$this->PluginVs_Stat_GetTournamentByTournamentId($tournament_id);
		
		
		$aFilter['my_team_id']=$this->PluginVs_Stat_GetMyTeam($oTournament);
		$aFilter['my_teamintournament_id'] = $this->PluginVs_Stat_GetMyTeamtournament($oTournament);
		
		if( isset($teams) &&  $teams && is_array($teams)){ 
			foreach ($teams as $selectedTeam){
				$aFilter['teams'][] =  intval($selectedTeam);
			}
		}else{
		...
		$aFilter['tournament_id'] = $tournament_id;		
		if($round != -1)$aFilter['round_id'] = $round;
		if($round_po != -1)$aFilter['round_po'] = $round_po;
		if($event != -1)$aFilter['event_id'] = $event;
			...
			if(isset($Setteam)) $oViewer->Assign('teambrief',$teambrief);
	...
	protected function EventGetPlay(){
		...
		$tournament=array();
		$tournament = $this->PluginVs_Stat_GetMyTournaments();
	...
	protected function EventRaspisanieFilter() 	{
		...
		$aFilter['team'] = $myteam;
		$oMatches=$this->PluginVs_Stat_MatchesSQLNew($myteam, $userid, $Setteam, $teambrief, $month, $Setcurrentweek, $week, $tournament_id);
	...
	protected function EvenAddMatchVideo(){ 
		...
		if($video_url){
			if($oMatchVideo = $this->PluginVs_Stat_GetMatchVideoByFilter(array(
					'match_id' => $match_id,
					'user_id' => $this->oUserCurrent->GetUserId()					
					)) 
			){
				$oMatchVideo->setLink($video_url); 	
				$oMatchVideo->setText( $this->Text_Parser("<video>".$video_url."</video>") ); 
				$oMatchVideo->setTimes(date("Y-m-d H:i:s"));
				$oMatchVideo->Save();
			}else{
				$MatchVideo_add = Engine::GetEntity('PluginVs_Stat_MatchVideo');
				$MatchVideo_add->setLink($video_url); 	
				$MatchVideo_add->setText( $this->Text_Parser("<video>".$video_url."</video>") ); 
				$MatchVideo_add->setTimes(date("Y-m-d H:i:s"));
				$MatchVideo_add->setUserId($this->oUserCurrent->GetUserId()); 
				$MatchVideo_add->setMatchId($match_id);
				$MatchVideo_add->Add();
			}
			...
			$this->Message_AddErrorSingle('Where is a link to video?',$this->Lang_Get('error'));
			return;			
		}
	}
	protected function EventGoFrom(){ 
		...
		if($who=='team' && $oPlayertournament->getTeamId()!=0 && $this->checkRight($oPlayertournament->getTeamId()) ){
	...
	protected function EventTeamplayNewStatus(){
		...
		if( ($oPlayer && $oPlayer->getCap()==2) || $this->oUserCurrent->getIsAdministrator()){		
			...
			{
				if(($oPlayer && $oPlayer->getTeamId()==$oPlayerTournament->getTeamId()) || $this->oUserCurrent->getIsAdministrator()){
					if($id!=2){
	...
	protected function EventToEvent(){
		if (func_check(getRequest('event_id',null,'post'),'id',1,11))$event_id=getRequest('event_id',null,'post');
		
		$oEvent=$this->PluginVs_Stat_GetEventByEventId($event_id);
		$oTeamTournament = $this->PluginVs_Stat_GetTeamsintournamentByFilter(array('tournament_id' => $oEvent->getTournamentId(),'player_id' => $this->oUserCurrent->getUserId()));
		//	$this->myTeamTournament = $oTeamTournament->getId();
		...
		{		
			$oEventPlayer = Engine::GetEntity('PluginVs_Stat_EventPlayer');
			$oEventPlayer->setEventId($event_id); 
			$oEventPlayer->setTeamintournamentId($oTeamTournament->getId());	
			$oEventPlayer->setDates(date("Y-m-d H:i:s"));	
			$oEventPlayer->Add();
			
		}else{
			$this->Message_AddErrorSingle(" ","Already");
		}
...
protected function EventPayVznos () {
	...
	if($oTournament=$this->PluginVs_Stat_GetTournamentByTournamentId($tournament_id) ){
		...
		if(count($aOperations)==0){			
			...
			$aResult = $this->PluginLspurse_Api_Pay($this->oUserCurrent, null,  $summa_vznosa,  0, null, 'Оплата участия в турнире '.$oTournament->getName().' '.$this->oUserCurrent->getLogin(), 'out', 'tournament', $tournament_id);
...
//-------------
	...
	protected function EventGetMatchButtons() {
		...
		if ($this->User_IsAuthorization()) {
			...
			if($myteam==0 && $oMatch->getGametypeId()==3){
				...
				'user_id' => $this->oUserCurrent->GetUserId()		
					...
					$myteam=$aTeam->getTeamId();
					$myteamtournament=$aTeam->getTeamintournamentId();
				}
			}
			$userid=$this->oUserCurrent->getId();
		...
		$oViewer->Assign('isAdmin',$isAdmin);
		$oViewer->Assign('myteam',$myteam);
		$oViewer->Assign('myteamtournament',$myteamtournament);
...
if($oTournament){		
	...
	protected function EventGetMoreEvents() {
		...
		$this->Viewer_AssignAjax('result', $oViewer->Fetch(Plugin::GetTemplatePath(__CLASS__)."actions/ActionTournament/events.tpl"));
	...
	protected function EventMatchResultDelete() 	{
		...
		if (func_check(getRequest('match_id',null,'post'),'id',1,11))$match_id=getRequest('match_id',null,'post');
		if (func_check(getRequest('team_id',null,'post'),'id',1,11))$team_id=getRequest('team_id',null,'post');
		if (func_check(getRequest('teamintournament_id',null,'post'),'id',1,11))$teamintournament_id=getRequest('teamintournament_id',null,'post');
		$oMatch = $this->PluginVs_Stat_GetMatchesByMatchId($match_id);
...
protected function EventMatchResultEdit() 	{
	//$this->Message_AddNoticeSingle("турнирная таблица создана",$this->Lang_Get('attention'));
		...
		$good=1;
		if($ot!=0 && $so!=0)$good=0;
		if(( $so!=0) && abs($away_goals-$home_goals)!=1)$good=0;
		...
		{	
			...
			if( $oTeamintournament = $this->PluginVs_Stat_GetTeamsintournamentByFilter(array(
				...
				$oMatchResult_add->setTeamId($oTeamintournament->getTeamId());
				$oMatchResult_add->setTeamintournamentId($oTeamintournament->getId());
	...
	protected function EventMatchTehMassovo() 	{
		...
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,3))$tournament_id=getRequest('tournament_id',null,'post');
		$sql="select m.match_id,
			case when t.away_w=1 then m.home_teamtournament else m.away_teamtournament end as teamtournament,
		...
		foreach($aMatches as $Match){			
			...
			$match_id = $Match['match_id'];
			$teamintournament_id = $Match['teamtournament'];
			$texts = $Match['text'];
			...
			if (isTournamentAdmin($oMatch->getTournamentId(),$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) 
			{
				...
				$user_id=$this->oUserCurrent->GetUserId();
				if( $oTeamintournament = $this->PluginVs_Stat_GetTeamsintournamentByFilter(array(
							'id' => $oMatch->getAwayTeamtournament(),
							'tournament_id' => $oMatch->getTournamentId()			
							))
					){				
						$user_id=$oTeamintournament->getPlayerId();
						if($user_id!=0 && ($oMatch->getAwayTeamtournament()==$teamintournament_id || $texts=='tehn')){
							$oPlayerTournament = $this->PluginVs_Stat_GetPlayertournamentByFilter(array(
							'user_id' => $user_id,
							'tournament_id'  => $oMatch->getTournamentId() ));
							$oPlayerTournament->setPenalties($oPlayerTournament->getPenalties()+1);
							...
							$oPenalty_add->setWhy('За технарь');
							$oPenalty_add->setDates(date("Y-m-d H:i:s"));	
							$oPenalty_add->Add();	
					...
					)
				{
					...
					$oMatchResult_add->setTeamintournamentId($oMatch->getAwayTeamtournament());
					$oMatchResult_add->setAway($away_goals);
					$oMatchResult_add->setHome($home_goals);
				...
				$user_id=$this->oUserCurrent->GetUserId();
				if($oTeamintournament = $this->PluginVs_Stat_GetTeamsintournamentByFilter(array(
							'id' => $oMatch->getHomeTeamtournament(),
							'tournament_id' => $oMatch->getTournamentId()			
							))
					){				
						$user_id=$oTeamintournament->getPlayerId();
						
						if($user_id!=0 && ($oMatch->getHomeTeamtournament()==$teamintournament_id || $texts=='tehn')){
							$oPlayerTournament = $this->PluginVs_Stat_GetPlayertournamentByFilter(array(
								'user_id' => $user_id,
								'tournament_id'  => $oMatch->getTournamentId() ));
							$oPlayerTournament->setPenalties($oPlayerTournament->getPenalties()+1);
					...
					)
				{
					...
					$oMatchResult_add->setTeamintournamentId($oMatch->getHomeTeamtournament());
					$oMatchResult_add->setAway($away_goals);
					$oMatchResult_add->setHome($home_goals);
	...
	//$this->Message_AddNoticeSingle("турнирная таблица создана",$this->Lang_Get('attention'));
		...
		if (func_check(getRequest('teamintournament_id',null,'post'),'id',1,11))$teamintournament_id=getRequest('teamintournament_id',null,'post');//prava dodelat
		if (func_check(getRequest('user_id',null,'post'),'id',1,11))$user_id=getRequest('user_id',null,'post');
		if (func_check(getRequest('zhaloba',null,'post'),'id',1,2))$zhaloba=getRequest('zhaloba',null,'post');
		if (func_check(getRequest('comment',null,'post'),'text',1,3000))$comment=getRequest('comment',null,'post');
		...
		$oTeaminTournament = $this->PluginVs_Stat_GetTeamsintournamentById($teamintournament_id);
		$team_id=$oTeaminTournament->getTeamId();		
...
*/			if($good==1){
						...
						)
				{
					...
					if($team_id!=0 || $teamintournament_id!=0){
						if($oMatch->getHomeTeamtournament()==$teamintournament_id){
							$oMatch->setHomeInsert(1);
							$oMatch->setHomeYard($yard);
							$oMatch->Save();
				...
				if($good==0)$this->Message_AddErrorSingle('Что-то не сходится в счете, либо разница мячей большая для SO, толи ещё что-то','Ошибка');
	...
	protected function EventMatchResultGet() 	{
		...
		if ($this->User_IsAuthorization()) {
			...
			$oMatch = $this->PluginVs_Stat_GetMatchesByFilter(array(
						'match_id' => $match_id, 
						'#with'         => array('hometeam','awayteam','homeuser','awayuser','hometeamtournament','hometeamtournament')
						));	
			if($oMatch->getTournamentId()!=0){
				if($oMatch->getHomeTeam()){
					$this->Viewer_AssignAjax('home_team',$oMatch->getHometeam()->getName());
				}else{					
					$this->Viewer_AssignAjax('home_team',$oMatch->getHometeamtournament()->getUser1()->getLogin());
				}
				if($oMatch->getAwayTeam()){
					$this->Viewer_AssignAjax('away_team',$oMatch->getAwayteam()->getName());
				}else{					
					$this->Viewer_AssignAjax('away_team',$oMatch->getAwayteamtournament()->getUser1()->getLogin());
				}
				$oTournament=$this->PluginVs_Stat_GetTournamentByTournamentId($oMatch->getTournamentId());	 
		...
		if ($this->User_IsAuthorization()) {
			...
			if( $oMatchResult = $this->PluginVs_Stat_GetMatchresultByFilter(array(
				...
				{					
					...
					$this->Viewer_AssignAjax('ot',$oMatchResult->getOt());
					$this->Viewer_AssignAjax('so',$oMatchResult->getSo());
					$this->Viewer_AssignAjax('ko',$oMatchResult->getKo());
					$this->Viewer_AssignAjax('tko',$oMatchResult->getTehko());
					$this->Viewer_AssignAjax('sub',$oMatchResult->getSubmission());
					$this->Viewer_AssignAjax('dec',$oMatchResult->getDecision());					
					$this->Viewer_AssignAjax('period',$oMatchResult->getPeriod());
					$this->Viewer_AssignAjax('comment',$oMatchResult->getComment());
					$this->Viewer_AssignAjax('yess',1);
				}
			...
			if($oMatch->getHomeTeamtournament()==$teamintournament_id)$this->Viewer_AssignAjax('yard',$oMatch->getHomeYard());
			if($oMatch->getAwayTeamtournament()==$teamintournament_id)$this->Viewer_AssignAjax('yard',$oMatch->getAwayYard());
...
FROM `tis_stat_leagueteams`
where league_id=8
*/	
	if( $aTeams = $this->PluginVs_Stat_GetLeagueteamsItemsByFilter(array(
					'league_id ' => $league
				))
			)
		{
...
$oViewer->Assign('oGame',$oGame);			
	...
	protected function EventAuRaspisanie() {
		...
		$oViewer->Assign('oTournament',$oTournament);
		$Rounds = $this->PluginVs_Stat_GetRoundItemsByFilter(array(
				));
		$aRounds = array();
		foreach($Rounds as $Round){	
					$aRounds[$Round->getRoundId()]=$Round;			
				}
		$oViewer->Assign('aRounds',$aRounds);
	...
	protected function EventAuTournament() {
			...
			if($aRounds){
				...
				if($this->PluginVs_Stat_GetAll($sql))$Rounds[count($Rounds)]='100';
	...
	protected function EventDeleteTeamTournament() {
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',null,'post');
		if (func_check(getRequest('teamtournament_id',null,'post'),'id',1,11))$teamintournament_id=getRequest('teamtournament_id',null,'post');
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			if($oTeamsintournament = $this->PluginVs_Stat_GetTeamsintournamentByFilter(array(
					...
					if ($aMatches = $this->PluginVs_Stat_GetMatchesItemsByFilter(array(
							'tournament_id'=> $tournament_id,
							'round_id' => $oTeamsintournament->getRoundId(),
							'#where' => array('(home_teamtournament = (?d) or away_teamtournament = (?d) ) ' => array($oTeamsintournament->getId(),$oTeamsintournament->getId())), 
							'played' => '1' )) 
						)
					{
						$this->Message_AddErrorSingle('Exist played matches',$this->Lang_Get('attention'));
						return;
					}else{
						$aMatches=$this->PluginVs_Stat_GetMatchesItemsByFilter(array(
							'tournament_id'=> $tournament_id,
	...
	protected function EventAuDeleteTeam() {
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			if($oTeamsintournament = $this->PluginVs_Stat_GetTeamsintournamentByFilter(array(
					...
					if($oTournament->getKnownTeams()==3 && $team_id==0){					
						...
						{
							$this->Message_AddErrorSingle('Exist matches',$this->Lang_Get('attention'));
							return;
						}
						
						if ($aEventPlayer = $this->PluginVs_Stat_GetEventPlayerItemsByFilter(array(
							//'tournament_id'=> $tournament_id,
							'teamintournament_id' => $oTeamsintournament->getId()
							)) 
						)
						{
							$this->Message_AddErrorSingle('Уже поданы заявки на участие в ивентах',$this->Lang_Get('attention'));
							return;
						}
					...
					if($oTournament->getKnownTeams()==3){
						$oTeamsintournament->setTeamId(0);
					}else{
	...
	protected function EventAuChangeTeam() {
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			if($oTournament->getKnownTeams()!=3){
				...
				$oTeamsintournament2 = $this->PluginVs_Stat_GetTeamsintournamentByFilter(array(
							'player_id' => $user2_id,
							'tournament_id' => $tournament_id					
						));
				
				$team_id = $oTeamsintournament->getTeamId();
				$oTeamsintournament->setTeamId( $oTeamsintournament2->getTeamId() );
				$oTeamsintournament2->setTeamId( $team_id );
				
				$oTeamsintournament->Save();
				$oTeamsintournament2->Save();
			}
		}else {
			$this->Message_AddErrorSingle('вы не админ',$this->Lang_Get('attention'));
			return;
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			if( $oTeam = $this->PluginVs_Stat_GetTeamByFilter(array(
				...
				if($round_id==0){
					...
					if($oTeamsintournamentGroupRound = $this->PluginVs_Stat_GetTeamsintournamentByFilter(array(
							'team_id' => $oTeam->getTeamId(),
							'tournament_id' => $tournament_id,
							'round_id' => 0
						))
					){
						$oTeamsintournament->setPlayerId($oTeamsintournamentGroupRound->getPlayerId()); 
						$oTeamsintournament->setRoundId($round_id); 
					}
				}
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			if($oTournament->getGametypeId()!=3){
				...
				if($oTournament->getKnownTeams()!=3){
					...
					/*if( $team_id ==0 || !$this->PluginVs_Stat_GetTeamsintournamentByFilter(array(
									'team_id' => $team_id, 
									'tournament_id' => $tournament_id					
								))
						){
	...
	protected function EventAuZayavki() {
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			$sql="select distinct team_id from tis_stat_teamsintournament where player_id<>0 and team_id<>0 and tournament_id=".$tournament_id;
			...
			$sql="select distinct lt.team_id from tis_stat_leagueteams lt, tis_stat_tournament t where t.league_id=lt.league_id and t.tournament_id=".$tournament_id;
			...
			if( $oTournament->getKnownTeams()==3 && $oTournament->getLeagueId()!=0 ){
				$aTeams = $this->PluginVs_Stat_GetTeamItemsByFilter(array(
					//'league_id' => $oTournament->getLeagueId(), 
					'team_id in ' =>$teams,
					'#order' =>array('name'=>'asc')
					));
					$oViewer->Assign('aTeams',$aTeams);
	...
	protected function EventDeleteNotPlayed() {
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',null,'post');
		if (func_check(getRequest('round',null,'post'),'id',1,5))$round=getRequest('round',null,'post');
		$oViewer=$this->Viewer_GetLocalViewer();
		$rounds=array();
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			if(!$this->PluginVs_Stat_GetAll($sql) && $matches_in_day>0 && $matches_to_win>0)
			{
					...
					{	
						...
						if($oPlayoff_1->getTeamintournamentId()<>0 && $oPlayoff_2->getTeamintournamentId()<>0)
						while($matches<$total_team_matches)
						{
							...
							if($def_matches_to_win>2){
								if($matches<2){
									...
									$oMatch_add->setHomeTeamtournament($oPlayoff_1->getTeamintournamentId());
									$oMatch_add->setAwayTeamtournament($oPlayoff_2->getTeamintournamentId());
								}
								if($matches>=2 && $matches<4){
									$oMatch_add->setHome($oPlayoff_2->getTeamId());
									$oMatch_add->setAway($oPlayoff_1->getTeamId());
									
									$oMatch_add->setHomeTeamtournament($oPlayoff_2->getTeamintournamentId());
									$oMatch_add->setAwayTeamtournament($oPlayoff_1->getTeamintournamentId());
								}
								if($matches==4){
									$oMatch_add->setHome($oPlayoff_1->getTeamId());
									$oMatch_add->setAway($oPlayoff_2->getTeamId());
	...
	protected function EventSaveRound() {
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			if($oTeamintournament )$team_id = $oTeamintournament->getTeamId();
			...
			$oPlayoff->setTeamintournamentId($teamintournament_id);
	...
	protected function EventAuEvent() {
		if (func_check(getRequest('tournament',null,'post'),'id',1,11))$tournament_id=getRequest('tournament',null,'post');
		$oViewer=$this->Viewer_GetLocalViewer();
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			$oViewer->Assign('aEvents',$aEvents);			
			
			$sTextResult=$oViewer->Fetch(Plugin::GetTemplatePath(__CLASS__)."admins/admin_event.tpl");
			$this->Viewer_AssignAjax('sText',$sTextResult);
	...
	protected function EventAuEditEventPair() {
		$titul = 0;
		if (func_check(getRequest('left_teamtournament_id',null,'post'),'id',1,11))$left_teamtournament_id=getRequest('left_teamtournament_id',null,'post');
		if (func_check(getRequest('right_teamtournament_id',null,'post'),'id',1,11))$right_teamtournament_id=getRequest('right_teamtournament_id',null,'post');
		if (func_check(getRequest('event_pair_id',null,'post'),'id',1,11))$event_pair_id=getRequest('event_pair_id',null,'post');
		if (func_check(getRequest('event_id',null,'post'),'id',1,11))$event_id=getRequest('event_id',null,'post');
		if (func_check(getRequest('titul',null,'post'),'id',1,11))$titul=getRequest('titul',null,'post');
		
		if($event_pair_id==0){
			$oEventPair =  Engine::GetEntity('PluginVs_Stat_EventPair');
			$oEventPair->setEventId($event_id);	
			$oEventPair->setLeftTeamintournamentId($left_teamtournament_id);
			$oEventPair->setRightTeamintournamentId($right_teamtournament_id);	
			$oEventPair->setTitul($titul);
			$oEventPair->Add();		
		}else{
			if( $oEventPair=$this->PluginVs_Stat_GetEventPairByEventPairId($event_pair_id)){
				
				$oEvent=$oEventPair->getEvent();
				
				if($oEventPair->getLeftTeamintournamentId()!=0 && $oEventPair->getRightTeamintournamentId()!=0){
					if( $oMatch = $this->PluginVs_Stat_GetMatchesByFilter(array(
						'event_id' => $event_id,	
						'tournament_id'=> $oEvent->getTournamentId(),
						'away_teamtournament'=>$oEventPair->getLeftTeamintournamentId(),
						'home_teamtournament'=>$oEventPair->getRightTeamintournamentId()
					)) ){

						if($oMatch->getPlayed()==1){
							$this->Message_AddErrorSingle('матч уже сыгран',$this->Lang_Get('attention'));
						}else{
							if($left_teamtournament_id!=0 && $right_teamtournament_id!=0){
								$oMatch->setAwayTeamtournament($left_teamtournament_id);
								$oMatch->setHomeTeamtournament($right_teamtournament_id);
								
								$oAwayTeamTournament = $this->PluginVs_Stat_GetTeamsintournamentById($left_teamtournament_id);
								$oHomeTeamTournament = $this->PluginVs_Stat_GetTeamsintournamentById($right_teamtournament_id);
								
								$oMatch->setAway($oAwayTeamTournament->getTeamId());
								$oMatch->setHome($oHomeTeamTournament->getTeamId());							
								$oMatch->setTitul($titul);
								$oMatch->Save();
							}else{
								$oMatch->Delete();						
							}
				...
				if( ($oEventPair->getLeftTeamintournamentId()==0 && $left_teamtournament_id!=0 && $oEventPair->getRightTeamintournamentId()!=0) 
					|| ($oEventPair->getRightTeamintournamentId()==0 && $right_teamtournament_id!=0 && $oEventPair->getLeftTeamintournamentId()!=0) )
				{
					$oTournament = $oEvent->getTournament();
					
					$sql="select max(number) as num from tis_stat_matches where tournament_id='".$oTournament->getTournamentId()."'  union select 0 as num order by num desc";
					if ($aNum=$this->PluginVs_Stat_GetAll($sql)) $num=$aNum[0]['num']; 
					$num++;
				
					$oMatch_add =  Engine::GetEntity('PluginVs_Stat_Matches');			
					$oMatch_add->setTournamentId($oTournament->getTournamentId());
					$oMatch_add->setEventId($oEvent->getEventId());
					$oMatch_add->setRoundId('0');
					$oMatch_add->setRoundPo('');
					$oMatch_add->setGameId($oTournament->getGameId());
					$oMatch_add->setGametypeId($oTournament->getGametypeId());
					$oMatch_add->setDates($oEvent->getDates());
					
					$oAwayTeamTournament = $this->PluginVs_Stat_GetTeamsintournamentById($left_teamtournament_id);
					$oHomeTeamTournament = $this->PluginVs_Stat_GetTeamsintournamentById($right_teamtournament_id);
					
					$oMatch_add->setAway($oAwayTeamTournament->getTeamId());
					$oMatch_add->setHome($oHomeTeamTournament->getTeamId());
							
					
					$oMatch_add->setAwayTeamtournament($oAwayTeamTournament->getId());
					$oMatch_add->setHomeTeamtournament($oHomeTeamTournament->getId());
					...
					$oMatch_add->setAwayComment('');
					$oMatch_add->setNumber($num);
					$oMatch_add->setTitul($titul);
					$oMatch_add->Add();
				
				}
				
				
				if( (isset($oMatch) && $oMatch && $oMatch->getPlayed()!=1) || !$oMatch ){
					$oEventPair->setLeftTeamintournamentId($left_teamtournament_id);
					$oEventPair->setRightTeamintournamentId($right_teamtournament_id);				
					$oEventPair->setTitul($titul);
					$oEventPair->Save();
		...
		$this->Viewer_AssignAjax('event_pair_id',$oEventPair->getEventPairId());
		$this->Viewer_AssignAjax('event_id',$oEventPair->getEventId());
	}
	
	
	protected function EventAuCalcEvent() {
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',null,'post');
		if (func_check(getRequest('event_id',null,'post'),'id',1,11))$event_id=getRequest('event_id',null,'post');
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			// ){
				$aStats = $this->PluginVs_Stat_GetTournamentstatItemsByFilter(array(
					'tournament_id' => $oEvent->getTournamentId(),
					'#with' => array('teamtournament','tournament')		
				));
				
				foreach($aStats as $oStat)
				{
					if(!$oRatingGametype = $this->PluginVs_Stat_GetRatingByFilter(array(
						'game_id' => $oStat->getTournament()->getGameId(),
						'gametype_id' => $oStat->getTournament()->getGametypeId(),
						'user_id' => $oStat->getTeamtournament()->getPlayerId()			
						))
					){
						$oRatingGametype = null;
						$oRatingGametype =  Engine::GetEntity('PluginVs_Stat_Rating');
						$oRatingGametype->setUserId($oStat->getTeamtournament()->getPlayerId());
						$oRatingGametype->setGameId($oStat->getTournament()->getGameId());
						$oRatingGametype->setGametypeId($oStat->getTournament()->getGametypeId());
						$oRatingGametype->setRating(1000);
						$oRatingGametype->setMatches(0);
						$oRatingGametype->Add();				
					}
					//echo $oStat->getTournament()->getGameId().'_'.$oStat->getTournament()->getGametypeId().'_'.$oStat->getTeamtournament()->getPlayerId().'   ';
					
					$rating = $oRatingGametype->getRating();
					if($oStat->getTeamtournament()->getTeamId()==0)$rating = $rating - 1000;
					if($oStat->getTitul()==1)$rating = $rating + 2000;
					$oStat->setPoints($rating);
					$oStat->Save();
					//$position++;
				}
				...
				foreach($aStats as $oStats)
				{
					if($oStats->getPosition()==0){
						$oStats->setPositionDiff(0);
					}else{
						$oStats->setPositionDiff($oStats->getPosition() - $position);
					}
					$oStats->setPosition($position);
					$oStats->Save();
					$position++;
				...
				foreach($aMatches as $oMatch){	
					$teamsintournament[] = $oMatch->getHomeTeamtournament();
					$teamsintournament[] = $oMatch->getAwayTeamtournament();
				}
				$aTournamentStat = $this->PluginVs_Stat_GetTournamentstatItemsByFilter(array(
					'tournament_id' => $oEvent->getTournamentId(),
					'#where' => array('teamintournament_id not in (?a)' => array( $teamsintournament)),
					));
				foreach($aTournamentStat as $oTournamentStat){
					$oTournamentStat->setPoints( $oTournamentStat->getPoints() - $oTournament->getPenaltyStay());
					$oTournamentStat->Save();
				}				
				
				$aPlayerStat = $this->PluginVs_Stat_GetPlayerstatItemsByFilter(array(
					'tournament_id' => $oEvent->getTournamentId(),
					'#where' => array('teamintournament_id not in (?a)' => array( $teamsintournament)),
					));
				foreach($aPlayerStat as $oPlayerStat){
					$oPlayerStat->setPoints( $oPlayerStat->getPoints() - $oTournament->getPenaltyStay());
					$oPlayerStat->Save();
				}	
			...
			//$oEvent->setClosed('1');
			$oEvent->Save();
			$this->Message_AddNoticeSingle(" ","Посчитали и не закрыли");		
		}	
	}
	
	protected function EventAuDeleteEvent() {
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',null,'post');
		if (func_check(getRequest('event_id',null,'post'),'id',1,11))$event_id=getRequest('event_id',null,'post');
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			if( !$aMatches=$this->PluginVs_Stat_GetMatchesItemsByFilter(array(
				...
				$oEvent->Delete();
				$this->Message_AddNoticeSingle(" ","Deleted");
			}else{
				$this->Message_AddErrorSingle(" ","Exist matches");			
			}			
		}	
	}
	protected function EventAuAddEvent() {
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',null,'post');
		if (func_check(getRequest('event_name',null,'post'),'text',1,150))$event_name=getRequest('event_name',null,'post');
		if (func_check(getRequest('event_date',null,'post'),'text',1,40))$event_date=getRequest('event_date',null,'post');
		
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			
			$oEvent =  Engine::GetEntity('PluginVs_Stat_Event');
			$oEvent->setTournamentId($tournament_id);	
			
			$res=explode('-', $event_date);
			$dates=mktime(0, 0, 0, $res[1], $res[0], $res[2]);
				
			$oEvent->setDates(date("Y-m-d", $dates));	
			$oEvent->setName($event_name);	
			$oEvent->Add();
	...
	protected function EventAuEventDay() {
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',null,'post');
		if (func_check(getRequest('event_id',null,'post'),'id',1,11))$event_id=getRequest('event_id',null,'post');
		
		$oEvent=$this->PluginVs_Stat_GetEventByEventId($event_id);
		
		if (isTournamentAdmin($oEvent->getTournamentId(),$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			$oViewer=$this->Viewer_GetLocalViewer();
			
			//$oEvent=$this->PluginVs_Stat_GetEventByEventId($event_id);
			$oTournament=$this->PluginVs_Stat_GetTournamentByTournamentId($oEvent->getTournamentId());
			
			$oViewer->Assign('oEvent',$oEvent);
			$oViewer->Assign('oTournament',$oTournament);
			
			$aEventPlayers=$this->PluginVs_Stat_GetEventPlayerItemsByFilter(array(
				'event_id' => $event_id,
				'#order' =>array('dates'=>'desc')
			));
			$oViewer->Assign('aEventPlayers',$aEventPlayers);
			 
			$aEventPairs=$this->PluginVs_Stat_GetEventPairItemsByFilter(array(
				'event_id' => $event_id
			));
			$oViewer->Assign('aEventPairs',$aEventPairs);
			...
			$sTextResult=$oViewer->Fetch(Plugin::GetTemplatePath(__CLASS__)."admins/event.tpl");
			$this->Viewer_AssignAjax('sText',$sTextResult);
		}
	}
	protected function EventAuEditEvent() {
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',null,'post');
		if (func_check(getRequest('event_name',null,'post'),'text',1,150))$event_name=getRequest('event_name',null,'post');
		if (func_check(getRequest('event_date',null,'post'),'text',1,40))$event_date=getRequest('event_date',null,'post');
		if (func_check(getRequest('event_id',null,'post'),'id',1,11))$event_id=getRequest('event_id',null,'post');
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			if( date("Y-m-d", $dates) != $oEvent->getDates()){
				$aMatches=$this->PluginVs_Stat_GetMatchesItemsByFilter(array(
					'event_id' => $event_id,	
					'tournament_id'=> $oEvent->getTournamentId()
				));
				foreach($aMatches as $oMatch){	
					$oMatch->setDates(date("Y-m-d", $dates));
					$oMatch->Save();
				}
	...
	protected function EventAuPlayoff() {
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {
			...
			$sql="SELECT num, team_id, teamintournament_id 	FROM `tis_stat_playoff` WHERE tournament_id = ".$tournament_id." and round='1' order by num";
			if( $aRoundPosition = $this->PluginVs_Stat_GetAll($sql)
					)
				{
					$aObjects=array();
					foreach($aRoundPosition as $oRoundPosition){	
						$aObjects[$oRoundPosition['num']]=$oRoundPosition['teamintournament_id'];			
			...
			$sql="SELECT num, team_id, teamintournament_id	FROM `tis_stat_playoff` WHERE tournament_id = ".$tournament_id." and round='1/2' order by num";
			if( $aRoundPosition = $this->PluginVs_Stat_GetAll($sql)
					)
				{
					$aObjects=array();
			...
			$sql="SELECT num, team_id,teamintournament_id	FROM `tis_stat_playoff` WHERE tournament_id = ".$tournament_id." and round='1/4' order by num";
			if( $aRoundPosition = $this->PluginVs_Stat_GetAll($sql)
					)
				{
					$aObjects=array();
			...
			$sql="SELECT num, team_id,teamintournament_id 	FROM `tis_stat_playoff` WHERE tournament_id = ".$tournament_id." and round='1/8' order by num";
			if( $aRoundPosition = $this->PluginVs_Stat_GetAll($sql)
					)
				{
					$aObjects=array();
			...
			$sql="SELECT num, team_id,teamintournament_id 	FROM `tis_stat_playoff` WHERE tournament_id = ".$tournament_id." and round='1/16' order by num";
			if( $aRoundPosition = $this->PluginVs_Stat_GetAll($sql)
					)
				{
					$aObjects=array();
					...
					$oViewer->Assign('Teams16',$aObjects);	
					$sql="SELECT distinct 1	FROM `tis_stat_matches` WHERE tournament_id = ".$tournament_id." and round_id=100 and round_po='1/16' ";
					if($this->PluginVs_Stat_GetAll($sql) )$oViewer->Assign('Matches16',1);
				}
				
			$sql="SELECT num, team_id,teamintournament_id 	FROM `tis_stat_playoff` WHERE tournament_id = ".$tournament_id." and round='1/32' order by num";
			if( $aRoundPosition = $this->PluginVs_Stat_GetAll($sql)
					)
				{
					$aObjects=array();
					...
					$oViewer->Assign('Teams32',$aObjects);	
					$sql="SELECT distinct 1	FROM `tis_stat_matches` WHERE tournament_id = ".$tournament_id." and round_id=100 and round_po='1/32' ";
					if($this->PluginVs_Stat_GetAll($sql) )$oViewer->Assign('Matches32',1);
				}
	...
	protected function EventAuPenalty() {
		if (func_check(getRequest('tournament',null,'post'),'id',1,11))$tournament_id=getRequest('tournament',null,'post');		
		if (func_check(getRequest('penalty_id',null,'post'),'id',1,11))$penalty_id=getRequest('penalty_id',null,'post');
			
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {		
		
			$aPenalty = $this->PluginVs_Stat_GetPenaltyItemsByFilter(array(
					'tournament_id' => $tournament_id, 
					'#with'         => array('user') 
					));
			...
			$oViewer=$this->Viewer_GetLocalViewer();
			$oViewer->Assign('Tournament',$tournament_id);
			$oViewer->Assign('aPenalty',$aPenalty);
			
			if($penalty_id){
				$oPenalty = $this->PluginVs_Stat_GetPenaltyById($penalty_id);
				$oViewer->Assign('oPenalty',$oPenalty);
			}
			
			$sTextResult=$oViewer->Fetch(Plugin::GetTemplatePath(__CLASS__)."admins/admin_penalty.tpl");
			$this->Viewer_AssignAjax('sText',$sTextResult);
		} else {
			$this->Message_AddErrorSingle('вы не админ',$this->Lang_Get('attention'));
			return;
	...
	protected function EventAuSavePenalty() {
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',null,'post');		
		if (func_check(getRequest('penalty_id',null,'post'),'id',1,11))$penalty_id=getRequest('penalty_id',null,'post');
		if (func_check(getRequest('user_login',null,'post'),'text',1,40))$user_login=getRequest('user_login',null,'post');
		if (func_check(getRequest('why',null,'post'),'text',1,540))$why=getRequest('why',null,'post');
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {		
				...
				)))
			{
				...
				$oPenalty->setWhy($why);	
				$oPenalty->setUserId($oUser->getUserId());	
				$oPenalty->setTournamentId($tournament_id);	
				$oPenalty->Save();
				
				$oPlayerTournament->setPenalties($oPlayerTournament->getPenalties()+1);
				$oPlayerTournament->Save();
			}else{
				$this->Message_AddErrorSingle('Давать штрафы игроку не играющему в турнире дурной тон',$this->Lang_Get('error'));
				return;
			}
	...
	protected function EventAuAddPenalty() {
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',null,'post');		
		if (func_check(getRequest('penalty_id',null,'post'),'id',1,11))$penalty_id=getRequest('penalty_id',null,'post');
		if (func_check(getRequest('user_login',null,'post'),'text',1,40))$user_login=getRequest('user_login',null,'post');
		if (func_check(getRequest('why',null,'post'),'text',1,550))$why=getRequest('why',null,'post');
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {		
				...
				)))
			{
				...
				$oPenalty_add =  Engine::GetEntity('PluginVs_Stat_Penalty');
				$oPenalty_add->setWhy($why);	
				$oPenalty_add->setUserId($oUser->getUserId());	
				$oPenalty_add->setTournamentId($tournament_id);	
				$oPenalty_add->setDates(date("Y-m-d H:i:s"));	
				$oPenalty_add->Add();
	...
	protected function EventAuDeletePenalty() {
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',null,'post');		
		if (func_check(getRequest('penalty_id',null,'post'),'id',1,11))$penalty_id=getRequest('penalty_id',null,'post');
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {		
				...
				)))
			{
				...
				$this->Message_AddErrorSingle('Беда, пиши Клаусу',$this->Lang_Get('error'));
				return;
			}
	...
	protected function EventAuAward() {
		if (func_check(getRequest('tournament',null,'post'),'id',1,11))$tournament_id=getRequest('tournament',null,'post');
		if (func_check(getRequest('medal_id',null,'post'),'id',1,11))$medal_id=getRequest('medal_id',null,'post');
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {		
			...
			$aMedals = $this->PluginVs_Stat_GetMedalsItemsByFilter(array(
					'tournament_id' => $tournament_id, 
					'#with'         => array('team','user','playercard')
					));
			$medals=array();
			...
			$oViewer=$this->Viewer_GetLocalViewer();
			$oViewer->Assign('Tournament',$tournament_id);
			$oViewer->Assign('aMedals',$aMedals);
			$oViewer->Assign('medals', $medals);
			// $oViewer->Assign('aTournament', $aTournament ); 
			if($medal_id){
				$oMedal = $this->PluginVs_Stat_GetMedalsByMedalId($medal_id);
				$oViewer->Assign('oMedal',$oMedal);
			}
			
			$sTextResult=$oViewer->Fetch(Plugin::GetTemplatePath(__CLASS__)."admins/admin_award.tpl");
			$this->Viewer_AssignAjax('sText',$sTextResult);
		} else {
			$this->Message_AddErrorSingle('вы не админ',$this->Lang_Get('attention'));
			return;
	...
	protected function CountMedals($oMedal, $plus_minus) {
	
		if( $oUser = $this->User_GetUserById($oMedal->getUserId()) ) {

			switch ($oMedal->getMedal()) {
				case 'gold':
					$oUser->setGold( $oUser->getGold() + $plus_minus );
					break;
				case 'silver':
					$oUser->setSilver( $oUser->getSilver() + $plus_minus );
					break;
				case 'bronze':
					$oUser->setBronze( $oUser->getBronze() + $plus_minus );
					break;
				default:
					$oUser->setUnknown( $oUser->getUnknown() + $plus_minus );
			}
			$this->User_SetUserMedals($oUser);
		}
	}
	
	protected function EventAuSaveAward() {
		...
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',null,'post');		
		if (func_check(getRequest('medal_id',null,'post'),'id',1,11))$medal_id=getRequest('medal_id',null,'post');
		if (func_check(getRequest('prise',null,'post'),'text',1,140))$prise=getRequest('prise',null,'post');
		if (func_check(getRequest('weight',null,'post'),'text',1,550))$weight=getRequest('weight',null,'post');
		if (func_check(getRequest('medal',null,'post'),'text',1,550))$medal=getRequest('medal',null,'post');
		if (func_check(getRequest('playercards',null,'post'),'text',1,2550))$playercards=getRequest('playercards',null,'post');
		if (func_check(getRequest('users',null,'post'),'text',1,1550))$users=getRequest('users',null,'post');
		if (func_check(getRequest('teams',null,'post'),'text',1,1550))$teams=getRequest('teams',null,'post');
		if (func_check(getRequest('medal_link',null,'post'),'text',1,550))$medal_link=getRequest('medal_link',null,'post');
		if (func_check(getRequest('medal_text',null,'post'),'text',1,550))$medal_text=getRequest('medal_text',null,'post');
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {		
			...
			if($oMedal_edit){
				...
				$this->CountMedals($oMedal_edit, -1);
				
				$oMedal_edit->setMedalText($medal_text);
				$oMedal_edit->setMedalLink($medal_link); 
				$oMedal_edit->setLogo($prise);
				$oMedal_edit->setMedal($medal);
				$oMedal_edit->setWeight($weight);
				...
				$oMedal_edit->setTeamId('0');
					...
					if($playercards){
						...
						foreach ($aUsers as $sUser) {
							$sUser=trim($sUser);
							$aPlayerUser=explode('^',$sUser);
							$sUser=trim($aPlayerUser[0]);					
							if ($sUser=='' ) {
								continue;
							}
							if ( $oPlayercard = $this->PluginVs_Stat_GetPlayercardByFilter(array(
	...
	protected function EventAuAddAward() {
			...
			if($users){
				...
				foreach ($aUsers as $sUser) {
					...
					if ($oUser=$this->User_GetUserByLogin($sUser) and $oUser->getActivate()==1) {
						...
						$oMedals_add =  Engine::GetEntity('PluginVs_Stat_Medals');
						$oMedals_add->setMedalText($medal_text); 
						$oMedals_add->setMedalLink($medal_link); 
						...
						$oMedals_add->setLogo($prise);
						$oMedals_add->setMedal($medal);
						$oMedals_add->setWeight($weight);
						$oMedals_add->Add();
						
						$this->CountMedals($oMedals_add, 1);
					}
				}
			}		
	...
	protected function EventAuDeleteAward() {
		if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',null,'post');		
		if (func_check(getRequest('medal_id',null,'post'),'id',1,11))$medal_id=getRequest('medal_id',null,'post');
			
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) {		
			
			if($oMedal = $this->PluginVs_Stat_GetMedalsByFilter(array(
				'medal_id' => $medal_id,
				'tournament_id' => $tournament_id
				))
			){				
				$oMedal->Delete();				
				$this->CountMedals($oMedal, -1);
			...
			if( /*$aZayvki = $this->PluginVs_Stat_GetZayvkiByFilter(array(
					...
					)
				{
					...
					foreach($aZayvki as $oZayvka){						
						...
						$aObjects[$oZayvka['user_id']]['haveteam']=$oZayvka['haveteam'];
	...
	protected function EventSettingPsn() { 
		...
		$Identifier_value = $this->User_getUserFieldValueByName($this->oUserCurrent->getId(), $platfroms[$oPlatform->getBrief()]);

		if(!$Identifier_value){
			$sql= "SELECT id FROM tis_user_field WHERE name ='".$platfroms[$oPlatform->getBrief()]."'";
			if ($aNum=$this->PluginVs_Stat_GetAll($sql)) $id=$aNum[0]['id'];
			$this->User_setUserFieldsValues($this->oUserCurrent->getId(), array( $id=> $PsnID), 1);			
		}
				...
				)))
		{
			...
			if($aTournamentAdmins = $this->PluginVs_Stat_GetTournamentadminItemsByFilter(array(
				'tournament_id' => $oTournament->getTournamentId(),
				'status' => 'admin'	)) 
			) {
				foreach($aTournamentAdmins as $oTournamentAdmin){
					$admins[]=$oTournamentAdmin->getUserId();
				}
				$sTitle = 'Пользователь ' . $this->oUserCurrent->getLogin().' подал заявку в турнир '.$oTournament->getName();
				$sText = 'Пользователь <a href="'.$this->oUserCurrent->getUserWebPath().'">'.$this->oUserCurrent->getLogin().'</a> подал заявку в турнир <a href="'.$oTournament->getUrlFull().'">'.$oTournament->getName().'</a>'; 
				$oTalk = $this->Talk_SendTalk($sTitle, $sText, $this->oUserCurrent, $admins, false, false);
			}
...
group by mt.miniturnir_id";
 /*
	...
	protected function EventSettingHutTeam(){
		...
		if (func_check(getRequest('team_logo',null,'post'),'text',1,50))$team_logo=getRequest('team_logo',null,'post');
...
protected function EventCreateShedule() {
	...
	if (func_check(getRequest('tournament_id',null,'post'),'id',1,11))$tournament_id=getRequest('tournament_id',null,'post');
	if (func_check(getRequest('date1','','post'),'text',9,11))$date1=getRequest('date1','','post');
	if (func_check(getRequest('date2','','post'),'text',9,11))$date2=getRequest('date2','','post');
	if (func_check(getRequest('date3','','post'),'text',9,11))$date3=getRequest('date3','','post');
	if (func_check(getRequest('date4','','post'),'text',9,11))$date4=getRequest('date4','','post');
	if (func_check(getRequest('date5','','post'),'text',9,11))$date5=getRequest('date5','','post');
	if (func_check(getRequest('date6','','post'),'text',9,11))$date6=getRequest('date6','','post');
	if (func_check(getRequest('date7','','post'),'text',9,11))$date7=getRequest('date7','','post'); 
	
	if (func_check(getRequest('sparenno',null,'post'),'id',1,11))$sparenno=getRequest('sparenno',null,'post');
	if (func_check(getRequest('obratka',null,'post'),'id',1,11))$obratka=getRequest('obratka',null,'post');
	if (func_check(getRequest('group_id',null,'post'),'id',1,11))$group_id=getRequest('group_id',null,'post');
	if (func_check(getRequest('round_id',null,'post'),'id',1,11))$round_id=getRequest('round_id',null,'post');
	if (func_check(getRequest('parentgroup_id',null,'post'),'id',1,11))$parentgroup_id=getRequest('parentgroup_id',null,'post');
	if (func_check(getRequest('protivnik_group_id',null,'post'),'id',1,11))$protivnik_group_id=getRequest('protivnik_group_id',null,'post');
	...
	{		
		...
		$secret_team=999999; 
		$protivnik_group_id=$protivnik_group_id;
		...
		//if($this->oUserCurrent->GetUserId()==4)
		if($protivnik_group_id==0){
			$matches=gen_rasp($tournament_id, $with_your_group, $with_alien_group, $calendar, $parentgroup_id, $group_id, $obratka, $protivnik_group_id, $round_id);
		}else{
			$matches=gen_rasp_by_groups($tournament_id, $with_your_group, $with_alien_group, $calendar, $another_group_id, $group_id, $obratka, $round_id);
		}
...
protected function EventCreateStatTable() {
	...
	*/
	...
	protected function EventUpdateRatings() {
		if (func_check(getRequest('team_id',null,'post'),'id',1,11))$team_id=getRequest('team_id',null,'post');
		if (func_check(getRequest('val',null,'post'),'id',1,3))$val=getRequest('val',null,'post');
		if (func_check(getRequest('what',null,'post'),'text',1,10))$what=getRequest('what',null,'post');
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) 
		{
			if($oTeam=$this->PluginVs_Stat_GetTeamByTeamId($team_id))
			{
				if($what && $what=='att')$oTeam->setAttackRating($val);
				if($what && $what=='mid')$oTeam->setMiddleRating($val);
				if($what && $what=='def')$oTeam->setDefenseRating($val);
	...
	protected function EventUpdateTournament() {
		...
		if (func_check(getRequest('win',null,'post'),'text',1,4))$win=getRequest('win',null,'post');
		if (func_check(getRequest('lose',null,'post'),'text',1,5))$lose=getRequest('lose',null,'post');
		if (func_check(getRequest('penalty_stay',null,'post'),'text',1,4))$penalty_stay=getRequest('penalty_stay',null,'post');
		...
		if (func_check(getRequest('rating_lfrm',null,'post'),'id',1,4))$rating_lfrm=getRequest('rating_lfrm',null,'post');
		...
		if($datezayavki=getRequest('datezayavki')) {
			if(func_check($datezayavki,'text',6,10) && substr_count($datezayavki,'-')==2) {
				list($d,$m,$y)=explode('-',$datezayavki);
				if(@checkdate($m,$d,$y)) {
		...
		if($dateopenrasp=getRequest('dateopenrasp')) {
			if(func_check($dateopenrasp,'text',6,10) && substr_count($dateopenrasp,'-')==2) {
				list($d,$m,$y)=explode('-',$dateopenrasp);
				if(@checkdate($m,$d,$y)) {					
		...
		if (isTournamentAdmin($tournament_id,$this->User_IsAuthorization(), $this->oUserCurrent->GetUserId()) || $this->oUserCurrent->isAdministrator()) 
		{
			if($oTournament=$this->PluginVs_Stat_GetTournamentByTournamentId($tournament_id))
			{
				...
				if(isset($penalty_stay))$oTournament->setPenaltyStay($penalty_stay);
				...
				if(isset($rating_lfrm))$oTournament->setRatingLfrm($rating_lfrm);
				$oTournament->Save();
			}			
		}
	}
	...
	protected function EventDeleteAloneTeams() {
		...
		{	
			...
			foreach($aTeamsintournament as $oTeamsintournament)
			{
					...
					)
				{
					foreach($aMatches as $oMatch){			
						...
						$this->PluginVs_Stat_DeleteStat($oMatch->getMatchId());
	...
	protected function EventUpdateStattable() {
		...
		foreach($aMatches as $Match){
		  ...
		  */
			...
			$aStats = array();               
			$oStat = array();
			if($aStats=$this->PluginVs_Stat_GetPlayerstatItemsByFilter(array(
					'tournament_id' => $tournament_id,
					'round_id' => $round_id
					)) ){
				foreach($aStats as $oStat)
			...
			$this->PluginVs_Stat_EventCreateStatTableByTournamentRound($tournament_id,$round_id);
...
//fwrite($fh, $oMatch->getMatchId().'-'); 
				$this->PluginVs_Stat_FormulaLfrmRating($oMatch);
				$this->PluginVs_Stat_CalcStat($oMatch->getMatchId());	
				$this->PluginVs_Stat_CalcPlayerStat($oMatch->getMatchId());	
				if($match_id==0)$match_id=$oMatch->getMatchId();
			}
